...
#     iterable = dl if ENABLE_WEBDATASET else distr_dl
#     with tqdm(enumerate(iterable), total=len(iterable), desc=f"Epoch {epoch}") as pbar:
#         for i, (text, images) in pbar:
#             if i % 10 == 0 and is_root:
#                 t = time.time()
...
for epoch in range(resume_epoch, EPOCHS):
    ...
    with tqdm(enumerate(iterable), total=total_iters, desc=f"Epoch {epoch}") as pbar:
        for i, (text, images) in pbar:
            if args.fp16:
                images = images.half()
            device = 'cuda' if torch.cuda.is_available() else 'cpu'
            ...
            # Collective loss, averaged across all processes
            avg_loss = distr_backend.average_all(loss)

            if i % 10 == 0 and is_root:
                t = time.time()  # Record the time every 10 iterations

            if i % 10 == 9 and is_root:
                sample_per_sec = BATCH_SIZE * 10 / (time.time() - t)
                print(epoch, i, f'sample_per_sec - {sample_per_sec}')

            # Update the tqdm postfix every iteration with the latest avg_loss
            pbar.set_postfix(loss=f"{avg_loss.item():.4f}")
            ...
            if is_root:
                log = {
                    ...
                    'iter': i,
                    'loss': avg_loss.item(),
                    "sample_per_sec": sample_per_sec if i % 10 == 9 else None  # Log this only when it's updated
                }
                wandb.log(log)
